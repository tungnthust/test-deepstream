...
int main(int argc, char *argv[]) {
  ...
  GstElement *postprocessor = NULL, *imagesaver = NULL, *kafka_producer = NULL;
  // --- ADD TRACKER ELEMENT ---
  GstElement *tracker = NULL;

  // ADD TEE ELEMENT
  GstElement *tee_element = NULL;
  GstElement *queue_for_kafka = NULL, *queue_for_tiler = NULL;
  ...
  kafka_producer = gst_element_factory_make("nvdskafka", "kafka_producer");
  if (!kafka_producer) {
    g_printerr("Unable to create kafka_producer (nvdskafka)\n");
    return -1;
  }
  ...
  g_object_set(G_OBJECT(kafka_producer), "brokers", "localhost:8912", NULL);
  g_object_set(G_OBJECT(kafka_producer), "topic", "instrusion_detection", NULL);
  ...
  // Create the tee and queue elements
  tee_element = gst_element_factory_make("tee", "tee");
  if (!tee_element) {
    g_printerr("Unable to create tee element\n");
    return -1;
  }

  queue_for_kafka = gst_element_factory_make("queue", "queue-kafka");
  if (!queue_for_kafka) {
    g_printerr("Unable to create queue for kafka\n");
    return -1;
  }

  queue_for_tiler = gst_element_factory_make("queue", "queue-tiler");
  if (!queue_for_tiler) {
    g_printerr("Unable to create queue for tiler\n");
    return -1;
  }
  ...
  gst_bin_add_many(GST_BIN(pipeline), 
                   pgie, tracker, nvdslogger, postprocessor, imagesaver,
                   tee_element, queue_for_kafka, kafka_producer,
                   queue_for_tiler, tiler, nvvidconv, nvosd, vidconv, 
                   encoder, muxer, sink, NULL);

  // Link the main pipeline up to the tee
  if (!gst_element_link_many(streammux, pgie, tracker, nvdslogger, 
                             postprocessor, imagesaver, tee_element, NULL)) {
    g_printerr("Elements could not be linked up to tee. Exiting.\n");
    return -1;
  }

  // Link the kafka branch (tee -> queue -> kafka_producer)
  if (!gst_element_link_many(tee_element, queue_for_kafka, kafka_producer, NULL)) {
    g_printerr("Could not link kafka branch. Exiting.\n");
    return -1;
  }

  // Link the main branch (tee -> queue -> tiler -> ... -> sink)
  if (!gst_element_link_many(tee_element, queue_for_tiler, tiler, nvvidconv, 
                             nvosd, vidconv, encoder, muxer, sink, NULL)) {
    g_printerr("Could not link main branch after tee. Exiting.\n");
    return -1;
  }
